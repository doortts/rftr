...
public class MyMoney {
    ...
    /**
     * basic 한문 화폐단위를 담고있는 MData
     */
    private static final MData mdata = new MData("money");
    ...
    /**
     * int 값 piMoney 의 금액을 한문으로 표시한다.
     * 
     * @param piMoney  int 값
     * @return  금액을 한문으로 변환한 String
     */
    public static String toKanjiMoney(int piMoney) {
        return toKanjiMoney((double)piMoney);
    }
    ...
    public static String toKanjiMoney(float pfMoney) {
        return toKanjiMoney((double)pfMoney);
    }

    /**
     * long 값 plMoney 의 금액을 한문으로 표시한다.
     * 
     * @param plMoney  long 값
     * @return  금액을 한문으로 변환한 String
     */
    public static String toKanjiMoney(long plMoney) {
        return toKanjiMoney((double)plMoney);
    }

    /**
     * String 값 psMoney 의 금액을 한문으로 표시한다.
     * 
     * @param psMoney  String 값
     * @return  금액을 한문으로 변환한 String
     */
    public static String toKanjiMoney(String psMoney) {
        String tempMoney = maskClear2(psMoney);
        return toKanjiMoney(Double.parseDouble(tempMoney));
    }

    /**
     * double 값 piMoney 의 금액을 한문으로 표시한다.
     * 
     * @param piMoney  double 값
     * @return  금액을 한문으로 변환한 String
     */
    public static String toKanjiMoney(double pdMoney) {
        String sMoney = toHanMoney(removeComma(toCommaFormat(pdMoney)));
        
        StringBuffer kanjiMoney = new StringBuffer();
        
        for ( int i = 0 ; i < sMoney.length() ; i++ ) 
 	       if ( mdata.get(sMoney.charAt(i) + "") != null )
  	          kanjiMoney.append(mdata.get(sMoney.charAt(i)+""));
        return kanjiMoney.toString();
    }
